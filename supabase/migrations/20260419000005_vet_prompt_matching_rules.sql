-- Tighten the vet_record AI prompt to fix the four problems Catherine
-- saw on Hops's first import against the Seven Mile certificate format:
--
--   1. Vaccine combos ("EEE/WEE/Tetanus/WNV/Flu/Rhino") were returned
--      as a single item_name instead of being split into one event per
--      catalog antigen.
--   2. The same combo was emitted 5x — the certificate lists each
--      vaccination in both a summary table and an event history, and
--      the AI recorded both.
--   3. Ceremonial suffixes ("Rabies Vaccine: Yearly Equine Booster")
--      blocked catalog matches that would otherwise succeed.
--   4. Non-recurring items ("Clean Sheath", "Wellness Exam") landed
--      as health_events when they're really husbandry / the visit
--      itself.
--
-- Only `body` is updated. `default_body` keeps the original seed so we
-- can always diff against what shipped.
