...
export default async function CatalogPage() {
  ...
  return (
    <div className="p-6 max-w-2xl space-y-8">
      ...
      {/* Per-Lesson Subscription Pricing (monthly model — ADR-0019) */}
      <section>
        <h2 className="text-sm font-bold text-[#191c1e] uppercase tracking-wide mb-0.5">Subscription — Per Lesson</h2>
        <p className="text-xs text-[#444650] mb-3">
          Per-lesson rate for the monthly billing model. Each month&apos;s invoice = this rate × the number of lesson dates in the month (excluding barn closures).
        </p>
        <div className="border border-[#c4c6d1]/40 rounded-lg overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#f0f2f7] border-b border-[#c4c6d1]/40">
                <th className="px-4 py-2 text-left text-xs font-semibold text-[#444650] uppercase tracking-wide">Rider Type</th>
                <th className="px-4 py-2 text-left text-xs font-semibold text-[#444650] uppercase tracking-wide">Default Price / Lesson</th>
              </tr>
            </thead>
