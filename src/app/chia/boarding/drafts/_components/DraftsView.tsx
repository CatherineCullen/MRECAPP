...
import type { DraftsSnapshot, DraftInvoice } from '../_lib/loadDrafts'
import { sendDraftInvoice, discardDraftInvoice } from '../actions'

/**
 * Drafts — review + batch-send step.
 *
 * Each card = one boarder-person's month-end draft invoice. Shows the
 * line items so the admin can spot weirdness before Stripe mails it.
 * "Preview" opens the Stripe-hosted draft in a new tab (full PDF-style
 * view with the customer's name/address).
 *
 * Batch-send loops client-side one-per-invoice so the admin sees a real
 * progress indicator ("5 of 18"). One Stripe failure doesn't stop the
 * batch — it's marked failed in the per-row status and the loop
 * continues.
 */
    ...
    <section className={`bg-white rounded border border-[#c4c6d1]/40 ${done ? 'opacity-60' : ''}`}>
      <header className="flex items-baseline gap-3 px-4 py-2.5 bg-[#f7f9fc] rounded-t border-b border-[#c4c6d1]/40">
        ...
        <span className="text-xs text-[#8c8e98]">
          {fmtDate(draft.periodStart)} – {fmtDate(draft.periodEnd)}
        </span>
      ...
      <div className="divide-y divide-[#e8ecf5]">
        {draft.lines.map(l => (
          <div key={l.id} className="flex items-baseline gap-2 px-4 py-1.5 text-xs">
            ...
            <span className={`font-mono ${l.isCredit ? 'text-[#8f3434]' : 'text-[#191c1e]'}`}>
      ...
      <footer className="flex items-center gap-2 px-4 py-2 border-t border-[#e8ecf5] bg-[#fafbfd] rounded-b">
        ...
        {status.state === 'error' && (
          <span className="text-xs text-[#8f3434] truncate" title={status.message}>
            {status.message}
          </span>
        )}
        {status.state === 'sent' && (
        ...
        <button
          ...
          onClick={onDiscard}
          disabled={busy || done}
          className="text-xs text-[#8f3434] hover:text-[#7a2a2a] disabled:opacity-30 disabled:cursor-not-allowed"
        >
        ...
        <button
          ...
          onClick={onSend}
          disabled={busy || done}
          className="px-3 py-1 text-xs font-semibold rounded bg-[#002058] text-white hover:bg-[#001540] disabled:opacity-30 disabled:cursor-not-allowed"
        >
...
export default function DraftsView({ snapshot }: { snapshot: DraftsSnapshot }) {
  ...
  async function sendOne(draft: DraftInvoice) {
    setRowStatus(s => ({ ...s, [draft.id]: { state: 'sending' } }))
    const res = await sendDraftInvoice({ invoiceId: draft.id })
    setRowStatus(s => ({
      ...s,
      [draft.id]: res.ok
  ...
  async function discardOne(draft: DraftInvoice) {
    if (!confirm(`Discard the draft for ${draft.billedToLabel}? Charges will return to Review & Allocate.`)) return

    setRowStatus(s => ({ ...s, [draft.id]: { state: 'discarding' } }))
    const probe = await discardDraftInvoice({ invoiceId: draft.id })

    if (probe.ok) return // single-invoice discard executed

    if (probe.error === 'cascade_required' && probe.cascade) {
      // Shared-split case: one or more other boarders' invoices touch the
      ...
      setRowStatus(s => {
        const next = { ...s }
        for (const c of probe.cascade!) next[c.invoiceId] = { state: 'discarding' }
        return next
      })
      const res = await discardDraftInvoice({ invoiceId: draft.id, confirmCascade: true })
      if (!res.ok) {
    ...
    setRowStatus(s => ({ ...s, [draft.id]: { state: 'error', message: probe.error } }))
  }
  ...
  function sendAll() {
    ...
    startBatch(async () => {
      // Serial loop — Stripe doesn't love parallel invoice finalizes on the
      // same customer, and serial gives us a clean progress counter.
    ...
    return (
      <div className="p-6">
        <div className="max-w-xl p-4 text-sm text-[#444650]">
          No draft invoices. Approve and generate from the <strong>Invoices</strong> tab to create drafts here.
        </div>
      </div>
    )
  }
